...
Status Configuration::parseFromFile(const std::string &filename) {
    ...
    do {
        if (fd == -1) {
            ...
            if (errno == EPERM) {
                status = Status::Error(ErrorCode::E_FILE_NOT_PERMISSION, filename.c_str());
                break;
            }
            status = Status::Error(ErrorCode::E_INTERNAL_ERROR, "Unknown error");
            break;
        }
        // get the file size
        auto len = ::lseek(fd, 0, SEEK_END);
        if (len == 0) {
            status = Status::Error(ErrorCode::E_FILE_CONTENT_IS_EMPTY, filename.c_str());
            break;
        }
        ::lseek(fd, 0, SEEK_SET);
...
Status Configuration::fetchAsInt(const char *key, int64_t &val) const {
    ...
    if (!iter->second.isInt()) {
        return Status::Error(ErrorCode::E_CONFIG_INVALID_ITEM_TYPE, key, "integer");
    }
...
Status Configuration::fetchAsDouble(const char *key, double &val) const {
    ...
    if (!iter->second.isDouble()) {
        return Status::Error(ErrorCode::E_CONFIG_INVALID_ITEM_TYPE, key, "double");
    }
...
Status Configuration::fetchAsBool(const char *key, bool &val) const {
    ...
    if (!iter->second.isBool()) {
        return Status::Error(ErrorCode::E_CONFIG_INVALID_ITEM_TYPE, key, "boolean");
    }
...
Status Configuration::fetchAsString(const char *key, std::string &val) const {
    ...
    if (!iter->second.isString()) {
        return Status::Error(ErrorCode::E_CONFIG_INVALID_ITEM_TYPE, key, "string");
    }
...
Status Configuration::fetchAsSubConf(const char *key, Configuration &subconf) const {
    ...
    if (!iter->second.isObject()) {
        return Status::Error(ErrorCode::E_CONFIG_WRONG_JSON_FORMAT, key);
    }
...
Status Configuration::fetchAsIntArray(
    ...
    for (auto& entry : iter->second) {
        try {
            ...
            return Status::Error(ErrorCode::E_INTERNAL_ERROR, ex.what());
        }
    }
    return Status::OK();
}
