...
/*static*/ Status TimeUtils::initializeGlobalTimezone() {
    ...
    if (!FLAGS_timezone_name.empty()) {
        ...
        return Status::Error(ErrorCode::E_EMPTY_TIMEZONE);
    }
}
...
/*static*/ StatusOr<DateTime> TimeUtils::dateTimeFromMap(const Map &m) {
    ...
    for (const auto &kv : m.kvs) {
        if (!kv.second.isInt()) {
            return Status::Error(ErrorCode::E_INVALID_DATA_TYPE, "Should be integer type");
        }
        if (kv.first == "year") {
...
StatusOr<Value> TimeUtils::toTimestamp(const Value &val) {
    ...
    if (val.isStr()) {
        ...
        if (dateTime.microsec != 0) {
            return Status::Error(ErrorCode::E_INVALID_DATA_TYPE, "The timestamp  only supports seconds unit");
        }
        ...
        return Status::Error(ErrorCode::E_INVALID_DATA_TYPE,
                folly::stringPrintf("Incorrect timestamp type: `%s'", val.toString().c_str()));
    }

    if (timestamp < 0 || (timestamp > kMaxTimestamp)) {
        return Status::Error(ErrorCode::E_INVALID_DATA_TYPE,
                folly::stringPrintf("Incorrect timestamp value: `%s'", val.toString().c_str()));
    }
    return timestamp;
}
}   // namespace time
