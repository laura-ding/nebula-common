...
StatusOr<std::string> NetworkUtils::getIPv4FromDevice(const std::string &device) {
    ...
    if (iter == result.value().end()) {
        return Status::Error(ErrorCode::E_NET_GET_IPV4_FAILED, device.c_str());
    }
    return iter->second;
}
...
StatusOr<std::unordered_map<std::string, std::string>> NetworkUtils::listDeviceAndIPv4s() {
    ...
    if (::getifaddrs(&iflist) != 0) {
        return Status::Error(ErrorCode::E_SYSTEM_CALL_FAILED, "getifaddrs", ::strerror(errno));
    }
    ...
    ::freeifaddrs(iflist);
    if (dev2ipv4s.empty()) {
        return Status::Error(ErrorCode::E_NET_GET_IPV4_FAILED, "ifaddrs");
    }
    return dev2ipv4s;
}
...
StatusOr<std::vector<HostAddr>> NetworkUtils::resolveHost(const std::string& host,
    ...
    if (getaddrinfo(host.c_str(), nullptr, &hints, &res) != 0) {
        return Status::Error(ErrorCode::E_NET_HOST_NOT_FOUND, host.c_str());
    }
...
StatusOr<std::vector<HostAddr>> NetworkUtils::toHosts(const std::string& peersStr) {
    ...
    for (auto& peerStr : peers) {
        ...
        if (pos == folly::StringPiece::npos) {
            return Status::Error(ErrorCode::E_NET_BAD_IP, addrPort.start());
        }
        ...
        try {
            ...
            return Status::Error(ErrorCode::E_NET_BAD_IP, addrPort.subpiece(pos + 1));
        }
