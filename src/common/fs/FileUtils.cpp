...
StatusOr<std::string> FileUtils::readLink(const char *path) {
    ...
    if (len == -1) {
        return Status::Error(ErrorCode::E_FILESYSTEM_CALL_FAILED,
                folly::stringPrintf("readlink %s: %s", path, ::strerror(errno)));
    }
...
StatusOr<std::string> FileUtils::realPath(const char *path) {
    ...
    if (buffer == NULL) {
        return Status::Error(ErrorCode::E_FILESYSTEM_CALL_FAILED,
                folly::stringPrintf("realpath %s: %s", path, ::strerror(errno)));
    }
...
StatusOr<uint64_t> FileUtils::free(const char* path) {
    ...
    if (err != 0) {
        return Status::Error(ErrorCode::E_FILESYSTEM_CALL_FAILED,
                folly::stringPrintf("Failed to get diskInfo of %s", path));
    }
...
void FileUtils::Iterator::dirNext() {
    ...
    if (dent == nullptr) {
        status_ = Status::Error(ErrorCode::E_FILESYSTEM_CALL_FAILED, "EOF");
        return;
    }
    entry_ = dent->d_name;
}
...
void FileUtils::Iterator::openFileOrDirectory() {
    ...
    if (type_ == FileType::DIRECTORY) {
        if ((dir_ = ::opendir(path_.c_str())) == nullptr) {
            status_ = Status::Error(ErrorCode::E_FILESYSTEM_CALL_FAILED,
                    folly::stringPrintf("opendir `%s': %s", path_.c_str(), ::strerror(errno)));
            return;
        }
        ...
        if (!fstream_->is_open()) {
            status_ = Status::Error(ErrorCode::E_FILESYSTEM_CALL_FAILED,
                    folly::stringPrintf("open `%s': %s", path_.c_str(), ::strerror(errno)));
            return;
        }
